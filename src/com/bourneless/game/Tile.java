...
public class Tile implements Serializable {
	...
	public void paint(Graphics2D g, ArrayList<LightSource> ls) {
		g.drawImage(image, pos.x + renderX, pos.y + renderY, tileSize,
				tileSize, null);

		if (showDebug) {
			if (tileRole == TileRoles.EMPTY) {
				g.setColor(emptyColor);
				g.drawString("EMPTY", pos.x + renderX + tileSize / 4, pos.y
						+ renderY + tileSize / 2);
			} else if (tileRole == TileRoles.BUILD) {
				g.setColor(buildColor);
				g.drawString("BUILD", pos.x + renderX + tileSize / 4, pos.y
						+ renderY + tileSize / 2);
			} else if (tileRole == TileRoles.FARM) {
				g.setColor(farmColor);
				g.drawString("FARM", pos.x + renderX + tileSize / 4, pos.y
						+ renderY + tileSize / 2);
			} else if (tileRole == TileRoles.PATH) {
				g.setColor(pathColor);
				g.drawString("PATH", pos.x + renderX + tileSize / 4, pos.y
						+ renderY + tileSize / 2);
			}
		...
		if (showRole) {
			...
			if (tileRole != TileRoles.EMPTY) {
				g.drawRect(rect.x + 1, rect.y + 1, rect.width - 2,
						rect.height - 2);
			}
		}
		...
		if (highlighted && showRole) {
			...
			g.drawRect(pos.x + renderX, pos.y + renderY, tileSize - 1,
					tileSize - 1);
		}
	}
