...
public class Tree extends Entity {
	...
	public Tree(Tile tile, Map map, boolean isSapling) {
		...
		this.rect = new Rectangle(pos.x, pos.y, treeTop.getWidth(), treeTop.getHeight());
	...
	public void paint(Graphics2D g) {
		if(!isSapling) {
			if(!cut) {
				g.drawImage(treeTop, pos.x + renderX, pos.y + renderY - treeBottom.getHeight(), null);
				g.drawImage(treeBottom, pos.x + renderX,  pos.y + renderY,  null);
			} else {
				g.drawImage(Main.resourceLoader.treeBottomCut, pos.x + renderX, pos.y + renderY, null);
			}
